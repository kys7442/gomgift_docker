...
**구현 방법**
1. 폐쇄몰 전용 카테고리 코드 규칙 설정 (CM 접두사: CM10, CM11, CM12)
2. 카테고리 생성 시 자동 접두사 부여
3. 카테고리 ID 검증 로직 수정 (숫자만 → 숫자 또는 CM+숫자)
...
**구현 방법**
1. 상품 복사 기능 구현 (복사된 상품 ID: 원본ID_CM폐쇄몰ID, 예: PROD001_CM1)
2. 복사 시 카테고리 자동 변경 (폐쇄몰 전용 카테고리로 배치)
3. 폐쇄몰 설정에서 선택 (카테고리 또는 상품코드 선택)
...
**설정 항목**
1. 기본 정보 (제목, 사용여부)
2. 운영 기간 (시작일시, 종료일시, 무제한 옵션)
3. 접근 권한 (회원 선택 / 레벨 선택 / 입장 코드)
4. 진열 상품 (카테고리 / 상품코드 / 파트너사)
5. 타이틀 이미지 (파일 업로드, 미리보기)
6. 스킨 선택
7. 가격 표시 방식 (매입가 / 마진가 / 객단가)
8. 할인률 및 원단위 처리
9. 결제 수단
10. 페이징 타입
...
### 3.1 파일 생성
- [ ] `shop/closedmall_list.php` 생성 (CM 카테고리만 표시)
...
### 9.1 테이블 생성
- [ ] `gom_mall_closed_mall_item_override` 테이블 생성
...
adm/shop_admin/
  ├── closedmalllist.php
  ├── closedmallform.php
  ├── closedmallformupdate.php
  ├── closedmall_table_create.php
  ├── closedmall_item_override_table_create.php
  ├── closedmall_item_price_form.php
  ├── closedmall_item_price_update.php
  ├── categoryform.php (수정)
  ├── categoryformupdate.php (수정)
  ├── categorylist.php (수정)
  └── ajax_admin_closed_mall_*.php
...
shop/
  ...
  ├── closedmall_item.php
  ├── closedmall_cart.php
  ├── closedmall_cartupdate.php
  ├── closedmall_orderform.php
  ├── closedmall_orderformupdate.php
  ├── closedmall_orderinquiry.php
  ├── closedmall_orderinquiryview.php
  ├── list.php (수정)
  └── itemlist.php (수정)
